  ...
  return (
    <Link href={href} key={blog.slug} className="flex flex-col">
      <div className="h-[270px] w-full relative rounded-2xl overflow-hidden mb-4">
        <Image
          src={`${process.env.NEXT_PUBLIC_API_BASE_URL}${blog?.HeroSinglePages.bgImage?.url || '/images/generalImages/fallback.jpg'}`}
      ...
      <div className="flex items-center justify-between mt-auto">
        <div className="flex items-center gap-2">
          ...
          <div className="text-sm text-Secondary-Text">
            <p>{blog.HeroSinglePages.author}</p>
            <p>
              {blog.HeroSinglePages.date} • {blog.HeroSinglePages.readTime}
            </p>
          </div>
        </div>
